      ...
      {place.reachId && showFlowData && (
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-600 dark:text-gray-400">Current Flow</span>
            {flowLoading ? (
          ...
          {/* Mini Flow Chart */}
          {!flowLoading && !flowError && miniChartData.length > 0 && (
            <div className="mt-2">
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs text-gray-600 dark:text-gray-400">12-Hour Trend</span>
                {peakFlow && peakFlow !== currentFlow && (
                  <span className="text-xs text-gray-500 dark:text-gray-400">
                    Peak: {formatFlow(peakFlow)}
                  </span>
                )}
              </div>
              
              {/* Simple SVG Mini Chart */}
              <div className="h-8 bg-gray-50 dark:bg-gray-700/50 rounded p-1">
                <svg 
                  width="100%" 
                  height="100%" 
                  viewBox="0 0 100 20" 
                ...
                >
                  {miniChartData.length > 1 && (
                    <>
                      ...
                      <polyline
                        fill="none"
                        stroke={riskLevel === 'normal' ? '#10b981' : riskLevel === 'flood' ? '#ef4444' : '#f59e0b'}
                        strokeWidth="1"
                        points={miniChartData
                          .map((flow, index) => {
                            const x = (index / (miniChartData.length - 1)) * 100;
                            const minFlow = Math.min(...miniChartData);
                            const maxFlow = Math.max(...miniChartData);
                            const range = maxFlow - minFlow || 1;
                            const y = 18 - ((flow - minFlow) / range) * 16;
                            return `${x},${y}`;
                          })
                          .join(' ')
                        }
                      ...
                      <circle
                        cx="0"
                        cy={18 - ((miniChartData[0] - Math.min(...miniChartData)) / (Math.max(...miniChartData) - Math.min(...miniChartData) || 1)) * 16}
                        r="1"
                        fill={riskLevel === 'normal' ? '#10b981' : riskLevel === 'flood' ? '#ef4444' : '#f59e0b'}
                      />
                    </>
                  )}
                </svg>
