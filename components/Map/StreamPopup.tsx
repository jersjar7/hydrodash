...
const StreamPopup: React.FC<StreamPopupProps> = ({
  ...
  const {
    ...
    error: flowError,
  } = useShortRangeForecast(streamData?.reachId || null, {
    enabled: isOpen && !!streamData?.reachId,
    staleTime: 2 * 60 * 1000, // 2 minutes - fresh data for current conditions
  });

  // Fetch return periods for risk calculation
  const {
    data: returnPeriodsData,
    loading: returnPeriodsLoading,
    error: returnPeriodsError,
    getReturnPeriods,
  } = useReturnPeriods(streamData?.reachId ? [streamData.reachId] : [], {
    enabled: isOpen && !!streamData?.reachId,
  });

  // Extract current flow and calculate risk level
  const currentFlow = getCurrentFlow(forecastData);
  const returnPeriods = streamData?.reachId ? getReturnPeriods(streamData.reachId) : null;
  
  // Calculate risk level using proper thresholds
  const riskLevel: RiskLevel = (() => {
    ...
    if (currentFlow !== null && returnPeriods && !returnPeriodsLoading) {
      try {
        return computeRisk(currentFlow, returnPeriods);
      } catch (error) {
        console.warn('Failed to compute risk level:', error);
        return 'normal';
      }
    }
    
    // Fallback to normal when we don't have enough data
    return 'normal';
  })();

  // Combine flow loading states
  ...
  return (
    ...
    >
      ...
      <div className="p-4 space-y-4">
        ...
        {/* Current Flow Status with integrated data fetching */}
        <div className="space-y-2">
          ...
          {flowDataLoading ? (
            ...
            <div className="p-3 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
              <p className="text-sm text-gray-500 dark:text-gray-400">
                No flow data available for this location
              </p>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                Stream may not have active monitoring
              </p>
            </div>
          )}

          {/* Show data freshness when we have valid data */}
          {hasFlowData && forecastData && (
            <p className="text-xs text-gray-400 dark:text-gray-500">
              Updated: {new Date().toLocaleTimeString()} • 
              {returnPeriods ? ' Risk calculated' : ' Using default thresholds'}
            </p>
          )}
        </div>
